  ...
  if (user) throw new Error("User has existed");
    ...
    if (newUser) {
      ...
      await sendMail({
        email,
        html,
        subject: "Confirm register account in MilkStore",
      });
    }
    setTimeout(async () => {
...
const getCurrent = asyncHandler(async (req, res) => {
  ...
  const user = await User.findById(_id)
    ...
    .populate("wishlist", "title thumb price typess");
  return res.status(200).json({
...
const updateCart = asyncHandler(async (req, res) => {
  const { _id } = req.user;
  const { pid, quantity = 1, typess, price, thumbnail, title } = req.body;
  if (!pid || !typess) throw new Error("Missing inputs");
  const user = await User.findById(_id).select("cart");
  const alreadyProduct = user?.cart?.find(
    (el) => el.product.toString() === pid && el.typess === typess
  );
  if (alreadyProduct) {
    ...
    const response = await User.findByIdAndUpdate(
      ...
      {
        $push: {
          cart: { product: pid, quantity, typess, price, thumbnail, title },
        },
      },
      { new: true }
    );
