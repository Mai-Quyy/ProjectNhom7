...
const Cart = ({ dispatch, navigate }) => {
  ...
  const removeCart = async (pid, typess) => {
    const response = await apiRemoveCart(pid, typess);
    if (response.success) {
      dispatch(getCurrent());
            ...
            <div key={el._id} className="flex justify-between items-center">
              <div className="flex gap-2">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-[10px]">{el.typess}</span>
              ...
              <span
                onClick={() => removeCart(el.product?._id, el.typess)}
